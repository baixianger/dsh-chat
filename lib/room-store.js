...
export class DshChatService {
    ...
    for (const member of recipients) {
      ...
      try {
        if (member.kind === "session") {
          ...
          bridge.deliverExternal(`chat:${room.id}:${message.author}`, member.sessionId, this.#agentDeliveryText(room.name, message), { id: message.id, transport: "chat" });
        } else {
  ...
  #agentDeliveryText(roomName, message) {
    const sender = message.authorAlias ?? message.author;
    return `[${roomName}] ${message.text}\n\n[DSH Chat delivery: a normal assistant reply stays only in this session and is not posted to the room. To answer ${sender} in the room, call chat_send with room "${roomName}" and mentions ["${sender}"].]`;
  }
  ...
  async #readHost(room, limit, waitMs) {
    ...
    await this.#save(); return reply.events;
  }
  ...
  async #receiveWeave(frame) {
    ...
    if (payload.kind === "room.read") {
      ...
      return { claimed: true, result: { cursor: room.messages.length, events: copy(events), members: room.members.map((member) => this.#memberView(member)) } };
    }
    if (payload.kind === "room.post") { const room = this.#room(payload.roomId); this.#authorize(room, payload.capability, payload.author, frame.peerId); const message = await this.send({ roomId: payload.roomId, author: payload.author, authorAlias: payload.authorAlias, text: payload.text, mentions: payload.mentions }); return { claimed: true, result: message }; }
    if (payload.kind === "room.delivery") {
      ...
      if (!bridge) throw new Error("dsh-bridge is not installed");
      bridge.deliverExternal(`chat:${payload.roomId}:${payload.message.author}`, payload.recipient, this.#agentDeliveryText(payload.roomName, payload.message), { id: payload.message.id, transport: "chat" });
      return true;
    }
    return false;
  }
