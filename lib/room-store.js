...
export class DshChatService {
  ...
  async addMember(roomId, member) {
    ...
    if (!room.members.some((item) => item.kind === normalized.kind && item.sessionId === normalized.sessionId && item.hostId === normalized.hostId)) room.members.push(normalized);
  ...
  async #acceptInvite(payload, hostId) {
    ...
    if (existing) { if (existing.hostId !== hostId) throw new Error("room invite host mismatch"); return; }
