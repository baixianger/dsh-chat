...
export class DshChatService {
  ...
  async listRooms() { await this.ready; return copy(this.state.rooms.map(({ messages, pendingDeliveries, ...room }) => ({ ...room, members: room.members.map((member) => this.#memberView(member)), messageCount: messages.length }))); }
