...
export class DshChatService {
  ...
  async #deliver(room, message) {
    const bridge = this.ctx?.dshBridge ?? this.ctx?.get?.("dshBridge"); const weave = this.ctx?.dshWeave ?? this.ctx?.get?.("dshWeave");
    const recipients = message.mentions.length === 0 || message.mentions.includes("all")
      ? room.members
