...
export class DshChatService {
      ...
      for (const member of room.members) {
        ...
        try {
          const status = member.kind === "session"
            ? await bridge?.status?.(member.sessionId)
  ...
  async createRoom({ name, members = [] }) {
    ...
    for (const member of room.members) if (member.kind === "remote") await this.#invite(room, member);
  ...
  async removeMember(roomId, member) {
    await this.ready; const room = this.#room(roomId);
  ...
  async send({ roomId, author, authorHostId, authorAlias, text, mentions }) {
    await this.ready;
    ...
    for (const member of recipients) {
      ...
      try {
        if (member.kind === "session") {
          ...
          await bridge.deliverExternal(`chat:${room.id}:${message.author}`, member.sessionId, this.#agentDeliveryText(room, message), { id: message.id, transport: "chat" });
        } else {
  ...
  async #receiveWeave(frame) {
    ...
    if (payload.kind === "room.post") {
      ...
      const message = await this.send({ roomId: payload.roomId, author, authorHostId: payload.author === roomViewAuthor ? undefined : frame.peerId, authorAlias: payload.authorAlias, text: payload.text, mentions: payload.mentions });
    ...
    if (payload.kind === "room.delivery") {
      ...
      const bridge = this.ctx?.dshBridge ?? this.ctx?.get?.("dshBridge");
      if (!bridge) throw new Error("dsh-bridge is not installed");
      await bridge.deliverExternal(`chat:${payload.roomId}:${payload.message.author}`, payload.recipient, this.#agentDeliveryText(room, payload.message), { id: payload.message.id, transport: "chat" });
      return true;
    }
    return false;
  }
