...
export class DshChatService {
  constructor(ctx, config = {}) {
    this.ctx = ctx;
    // The schema defaults these for a composed plugin; a directly constructed
    // store resolves the same directory from `$DSH_HOME`, then `~/.dsh`.
    const home = process.env.DSH_HOME || join(homedir(), ".dsh");
    this.path = config.path ?? join(home, "dsh-chat", "rooms.json");
  ...
  async #load() {
    try {
      ...
      if (parsed?.version === 1 && Array.isArray(parsed.rooms)) {
        this.state = parsed;
        await chmod(dirname(this.path), 0o700); await chmod(this.path, 0o600);
      }
    } catch (error) {
  ...
  attachWeave(attempt = 0) {
    if (this.closed || this.unsubscribeWeave) return;
    const weave = this.#weave();
    if (weave) {
      this.unsubscribeWeave = weave.subscribe((frame, options) => this.#receiveWeave(frame, options));
    ...
    // Weave is provided by an independently activated Cordis plugin. Retry
    // briefly so local rooms can start without making it a load prerequisite.
    if (attempt < 100 && !this.attachRetryTimer) {
      this.attachRetryTimer = setTimeout(() => {
        this.attachRetryTimer = undefined;
        this.attachWeave(attempt + 1);
      }, 100);
  ...
  async close() {
    ...
    this.attachRetryTimer = undefined;
    this.pendingRetryTimer = undefined;
    this.unsubscribeWeave?.();
    this.unsubscribeWeave = undefined;
    this.listeners.clear();
    await Promise.allSettled([this.saveTail, this.roomSessionTail, this.aliasRefresh].filter(Boolean));
  }

  #signal(options) { return AbortSignal.any([this.lifetime.signal, ...(options?.signal ? [options.signal] : [])]); }

  async listRooms(options) {
    const signal = this.#signal(options);
    await withSignal(this.ready, signal); signal.throwIfAborted();
    ...
    const weave = this.#weave();
    const rooms = [];
    for (const { messages, pendingDeliveries, ...room } of this.state.rooms) {
      const members = [];
      for (const member of room.members) {
        const view = this.#memberView(member);
        try {
          const status = member.kind === "session"
            ? await withSignal(bridge?.status?.(member.sessionId, signal), signal)
            : weave?.sessionStatus?.(member.hostId, member.sessionId);
          if (status?.state) view.state = status.state;
        } catch {
          signal.throwIfAborted();
          // A status probe only decorates the row: an unreachable Bridge/Weave peer
          // leaves the member without a state instead of failing the whole listing.
          this.ctx?.logger?.debug?.(`dsh-chat could not read the state of member ${member.sessionId}`);
        }
        members.push(view);
      }
      rooms.push({ ...room, members, messageCount: messages.length });
    }
    return copy(rooms);
  }
  async messages(roomId, limit = 100, waitMs = 0, options) {
    const signal = this.#signal(options);
    await withSignal(this.ready, signal); signal.throwIfAborted(); const room = this.#room(roomId);
    if (room.hostId || room.hostTicket) return this.#readHost(room, limit, waitMs, signal);
    return copy(room.messages.slice(-Math.max(1, Math.min(500, limit))));
  }

  async createRoom({ name, members = [] }, options) {
    const signal = this.#signal(options);
    await withSignal(this.ready, signal); signal.throwIfAborted();
    const roomName = ensureText(name, "room name");
    const existing = this.state.rooms.find((item) => item.name === roomName);
    // Rooms are addressed by name in chat_join/chat_send, so a second room with the
    // same name would make every later by-name reference ambiguous.
    if (existing) throw new Error(`room "${roomName}" already exists (${existing.id}); join it instead of creating a second one`);
    const room = { id: crypto.randomUUID(), name: roomName, createdAt: Date.now(), members: [], messages: [], pendingDeliveries: [], cursor: 0, messageOffset: 0 };
    for (const member of members) {
      const normalized = this.#member(member);
      if (normalized.kind === "remote") {
        if (!this.#weave()) throw new Error("dsh-weave is not installed");
        normalized.capability ??= crypto.randomUUID();
      }
      room.members.push(normalized);
    }
    this.state.rooms.push(room); await this.#save(); this.#emit({ kind: "room-created", room: { ...room, messages: [] } });
    await withSignal(this.ensureRoomSessions(), signal);
    for (const member of room.members) if (member.kind === "remote") await this.#invite(room, member, signal);
  ...
  async remoteSessions(options) {
    const signal = this.#signal(options); signal.throwIfAborted();
    const weave = this.#weave();
    if (!weave?.remoteSessions) return [];
    return (await withSignal(weave.remoteSessions(3_000, { signal }), signal)).map((host) => ({
      ...host,
  ...
  ensureRoomSessions() {
    const operation = this.roomSessionTail.then(async () => {
      ...
      const persistence = typeof this.ctx?.get === "function" ? this.ctx.get("sessionPersistence") : this.ctx?.sessionPersistence;
      ...
      for (const room of this.state.rooms) {
        ...
        if (!stored && (!room.sessionId || persistence)) {
          if (!session) {
            const time = Date.now();
          ...
          const existingLink = events?.find?.((event) => event.type === "chat/room-link");
          ...
          if (persistence) {
            // sessions.create publishes a live Session; only the storage handle
            // persists its seed. A plain room has no AgentLoop to own that write.
            const handle = await persistence.create(session.header, { inheritedEventCount: session.inheritedEventCount ?? 0 });
            try {
              await handle.append(typeof session.snapshotEvents === "function" ? session.snapshotEvents() : session.events);
              await handle.flush();
            } finally { await handle.close(); }
          } else { await sessions.flush?.(session); }
        }
  ...
  async resolveRoom(reference, options) {
    const signal = this.#signal(options);
    await withSignal(this.ready, signal); signal.throwIfAborted();
  ...
  async addMember(roomId, member, options) {
    const signal = this.#signal(options);
    await withSignal(this.ready, signal); signal.throwIfAborted(); const room = this.#room(roomId); const normalized = this.#member(member);
    ...
    if (existing) {
      ...
      await this.#save();
      if (existing.kind === "remote") await this.#invite(room, existing, signal);
    ...
    if (normalized.kind === "remote") await this.#invite(room, normalized, signal);
    return copy(normalized);
  }

  /** Remove one member durably. The room's authoritative host owns membership;
   * a linked room must ask its host. Absent members resolve without writing.
   * Pending deliveries to the removed member are dropped. */
  async removeMember(roomId, member, options) {
    const signal = this.#signal(options);
    await withSignal(this.ready, signal); signal.throwIfAborted(); const room = this.#room(roomId);
    if (room.hostId) throw new Error("only the room's host can remove members");
    if (!member || (member.kind !== "session" && member.kind !== "remote")) throw new TypeError("member.kind must be session or remote");
    const sessionId = ensureText(member.sessionId, "member sessionId");
    const hostId = member.kind === "remote" ? ensureText(member.hostId ?? "", "remote hostId") : undefined;
    const index = room.members.findIndex((item) => item.kind === member.kind && item.sessionId === sessionId && (hostId === undefined || item.hostId === hostId));
    if (index === -1) return null;
    const [removed] = room.members.splice(index, 1);
    if (Array.isArray(room.pendingDeliveries)) {
      const reference = memberReference(removed);
      room.pendingDeliveries = room.pendingDeliveries.filter((pending) =>
        pending.recipientRef ? pending.recipientRef !== reference : !(pending.recipient === sessionId && pending.hostId === hostId)
      );
    }
    await this.#save(); this.#emit({ kind: "member-removed", roomId, member: this.#memberView(removed) });
    return copy(removed);
  }

  async send({ roomId, author, authorHostId, authorAlias, text, mentions }, options) {
    const signal = this.#signal(options);
    await withSignal(this.ready, signal); signal.throwIfAborted();
    const room = this.#room(roomId);
    if (room.hostId || room.hostTicket) return this.#postToHost(room, { author, authorAlias, text, mentions }, signal);
    const authorId = ensureText(author, "author");
    const member = room.members.find((item) => item.sessionId === authorId && (
      authorHostId === undefined ? item.kind === "session" : item.kind === "remote" && item.hostId === authorHostId
    ));
    ...
    if (authorHostId !== undefined) message.authorHostId = ensureText(authorHostId, "author host id");
    ...
    // Retain the message and completed deliveries even when a later recipient is cancelled.
    try { await this.#deliver(room, message, signal); }
    finally { await this.#save(); this.#emit({ kind: "message", message }); }
    return copy(message);
  }
  ...
  #weave() {
    // Cordis property access requires inject even for an absent service.
    // Use its optional lookup without falling back to the throwing accessor;
    // direct properties remain supported for standalone stores.
    return typeof this.ctx?.get === "function" ? this.ctx.get("dshWeave") : this.ctx?.dshWeave;
  }
  ...
  async #refreshAliases() {
    const weave = this.#weave();
    if (!weave?.remoteSessions) return;
    const signal = this.lifetime.signal; signal.throwIfAborted();
    const hosts = await withSignal(weave.remoteSessions(3_000, { signal }), signal);
    signal.throwIfAborted();
    const catalog = new Map();
    for (const host of hosts) for (const workspace of host.workspaces ?? []) for (const session of workspace.sessions ?? []) {
      catalog.set(`${host.hostId}:${session.id}`, { alias: session.displayTitle ?? session.title, workspaceTitle: workspace.title, hostName: host.hostName });
    }
    let changed = false;
    for (const room of this.state.rooms) for (const member of room.members) {
      if (member.alias) continue;
      const entry = member.kind === "remote" ? catalog.get(`${member.hostId}:${member.sessionId}`) : undefined;
  ...
  #mentions(room, mentions) {
    ...
    const resolved = mentions.map((item) => {
      ...
      if (byId.length > 1) throw new Error(`session mention "${reference}" is ambiguous; choose it from the member list`);
      ...
      return memberReference(matches[0]);
    });
  ...
  async #deliver(room, message, signal) {
    ...
    const recipients = message.mentions.includes("all")
      ? room.members
      : room.members.filter((member) => message.mentions.includes(memberReference(member)));
    for (const member of recipients) {
      signal.throwIfAborted();
      if (member.sessionId === message.author && (
      ...
      try {
        if (member.kind === "session") {
          ...
          await bridge.deliverExternal(`chat:${room.id}:${message.author}`, member.sessionId, this.#agentDeliveryText(room, message), { id: message.id, transport: "chat", signal });
        } else {
          if (!weave) throw new Error("dsh-weave is not installed");
          await this.#deliverRemote(room, member, message, undefined, signal);
        }
        message.deliveries.push({ member: member.sessionId, kind: member.kind, status: "delivered" });
      } catch (error) { message.deliveries.push({ member: member.sessionId, kind: member.kind, status: "failed", error: String(error.message ?? error) }); signal.throwIfAborted(); }
    }
  }
  ...
  async #invite(room, member, signal) {
    signal.throwIfAborted();
    const weave = this.#weave();
    await withSignal(weave.sendTo({ signal, hostId: member.hostId, from: `chat:${room.id}:invite`, to: WEAVE_TARGET, text: JSON.stringify({ protocol: WEAVE_PROTOCOL, kind: "room.invite", room: { id: room.id, name: room.name, members: room.members.map((item) => this.#memberView(item)) }, recipient: member.sessionId, recipientAlias: member.alias, capability: member.capability }) }), signal);
  }
  async #deliverRemote(room, member, message, pending, signal = this.lifetime.signal) {
    signal.throwIfAborted();
    const weave = this.#weave();
    const record = pending ?? { id: message.id, recipient: member.sessionId, recipientRef: memberReference(member), hostId: member.hostId, message, createdAt: Date.now(), expiresAt: Date.now() + DELIVERY_TTL_MS, attempts: 0 };
    if (!pending) { room.pendingDeliveries.push(record); await this.#save(); }
    try {
      signal.throwIfAborted();
      record.attempts += 1;
      await withSignal(weave.sendTo({ signal, hostId: member.hostId, from: `chat:${room.id}:${message.author}`, to: WEAVE_TARGET, text: JSON.stringify({ protocol: WEAVE_PROTOCOL, kind: "room.delivery", roomId: room.id, roomName: room.name, message, recipient: member.sessionId, capability: member.capability }), id: message.id }), signal);
      room.pendingDeliveries = room.pendingDeliveries.filter((item) => item !== record);
      await this.#save();
    } catch (error) {
      // A user-cancelled send must not resume through the background retry queue.
      // Shutdown keeps already durable work for the next host run.
      if (signal.aborted && !this.closed) room.pendingDeliveries = room.pendingDeliveries.filter((item) => item !== record);
      await this.#save();
      throw error;
    }
  }
  #schedulePendingRetry() {
    if (this.closed || this.pendingRetryTimer) return;
    this.pendingRetryTimer = setTimeout(async () => {
      this.pendingRetryTimer = undefined;
      try { await this.#retryPendingDeliveries(); } catch (error) { if (!this.closed) this.ctx?.logger?.debug?.(String(error)); } finally { this.#schedulePendingRetry(); }
    }, 30_000);
  ...
  async #retryPendingDeliveries() {
    ...
    for (const room of this.state.rooms) {
      ...
      for (const pending of [...room.pendingDeliveries]) {
        ...
        const member = room.members.find((item) => item.kind === "remote" && (
          pending.recipientRef
            ? memberReference(item) === pending.recipientRef
            : item.sessionId === pending.recipient && (!pending.hostId || item.hostId === pending.hostId)
        ));
        if (!member) { room.pendingDeliveries = room.pendingDeliveries.filter((item) => item !== pending); changed = true; continue; }
        try {
          await this.#deliverRemote(room, member, pending.message, pending);
        } catch (error) {
          // A retry stays pending for the next attempt: an offline peer is an expected
          // outcome here, and the TTL sweep drops the record once it expires.
          this.ctx?.logger?.debug?.(`dsh-chat pending delivery to ${member.sessionId} failed: ${String(error)}`);
        }
      }
      if (changed) await this.#save();
    }
  }
  async #readHost(room, limit, waitMs, signal) {
    ...
    const reply = await this.#request(room.hostId, { kind: "room.read", roomId: room.id, capability: room.capability, cursor: room.cursor ?? 0, limit: Math.max(1, Math.min(500, Number(limit) || 100)), waitMs: Math.max(0, Math.min(MAX_READ_WAIT_MS, Number(waitMs) || 0)) }, signal);
    signal.throwIfAborted();
  ...
  async #postToHost(room, request, signal) {
    return this.#request(room.hostId, { kind: "room.post", roomId: room.id, capability: room.capability, sender: room.linkedSessionId, ...request }, signal);
  }
  async #request(hostId, payload, signal) {
    signal.throwIfAborted();
    const weave = this.#weave(); if (!weave) throw new Error("dsh-weave is not installed");
    const sent = await withSignal(weave.sendTo({ signal, hostId, from: "chat-client", to: WEAVE_TARGET, text: JSON.stringify({ protocol: WEAVE_PROTOCOL, ...payload }), timeoutMs: Math.max(10_000, (payload.waitMs || 0) + 5_000) }), signal);
    return sent.result;
  }
  async #receiveWeave(frame, options) {
    const signal = this.#signal(options); signal.throwIfAborted();
    if (frame.to !== WEAVE_TARGET) return false; let payload; try { payload = JSON.parse(frame.text); } catch { return false; }
    if (payload.protocol !== WEAVE_PROTOCOL) return false; await withSignal(this.ready, signal); signal.throwIfAborted();
    ...
    if (payload.kind === "room.read") {
      ...
      let events = read();
      if (events.length === 0 && payload.waitMs) { await this.#waitForMessages(room.id, cursor, payload.waitMs, signal); events = read(); }
    ...
    if (payload.kind === "room.post") {
      ...
      const message = await this.send({ roomId: payload.roomId, author, authorHostId: payload.author === roomViewAuthor ? undefined : frame.peerId, authorAlias: payload.authorAlias, text: payload.text, mentions: payload.mentions }, { signal });
    ...
    if (payload.kind === "room.delivery") {
      ...
      const bridge = this.ctx?.dshBridge ?? this.ctx?.get?.("dshBridge");
      if (!bridge) throw new Error("dsh-bridge is not installed");
      await bridge.deliverExternal(`chat:${payload.roomId}:${payload.message.author}`, payload.recipient, this.#agentDeliveryText(room, payload.message), { id: payload.message.id, transport: "chat", signal });
      return true;
    }
    return false;
  }
  ...
  #waitForMessages(roomId, cursor, waitMs, signal) {
    signal.throwIfAborted();
    return new Promise((resolve) => {
      let unsubscribe;
      const done = () => { clearTimeout(timer); unsubscribe?.(); signal.removeEventListener("abort", abort); resolve(); };
      const abort = () => { done(); };
