...
export class DshChatService {
  ...
  attachWeave(attempt = 0) {
    ...
    if (weave) {
      ...
      void this.ready.then(async () => {
        const tickets = new Set(this.state.rooms.flatMap((room) => room.members ?? []).filter((member) => member.kind === "weave" && member.ticket).map((member) => member.ticket));
        await Promise.all([...tickets].map((ticket) => weave.trust(ticket)));
      }).catch((error) => this.ctx?.logger?.warn?.(`dsh-chat could not restore a paired Weave ticket: ${String(error)}`));
  ...
  /** Public, non-sensitive session roster shared only with trusted Weave peers. */
  async sessionCatalog() {
    ...
    for (const header of headers) {
      ...
      if (id.startsWith(ROOM_SESSION_PREFIX) || id.startsWith("dsh-chat-room-")) continue;
      ...
      if (!title && persistence) {
        try {
          ...
          const titleEvent = [...inspected.events].reverse().find((event) => event.type === "session/title");
    ...
    return { hostName: this.hostName, sessions: items.sort((a, b) => b.updatedAt - a.updatedAt) };
  }

  /** Query paired, reachable Weave hosts for their public session rosters. */
  async remoteSessions() {
    const weave = this.#weave();
    const endpoints = weave?.endpoints?.() ?? [];
    const catalogs = await Promise.all(endpoints.map(async ({ peerId, ticket }) => {
      try {
        const sent = await Promise.race([
          weave.send({ ticket, from: "chat-session-picker", to: WEAVE_TARGET, text: JSON.stringify({ protocol: WEAVE_PROTOCOL, kind: "session.catalog" }) }),
          new Promise((_, reject) => setTimeout(() => reject(new Error("peer catalog timed out")), 3_000))
        ]);
        if (!sent?.result || !Array.isArray(sent.result.sessions)) return undefined;
        return { peerId, ticket, hostName: String(sent.result.hostName || peerId.slice(0, 8)), sessions: sent.result.sessions };
          ...
          if (!session) {
            ...
            session = sessions.create(sessionId, {
              ...
              seed: [
                ...
                { type: "chat/room-link", seq: 1, time: time + 1, data: { roomId: room.id, name: room.name, remote: Boolean(room.hostTicket) }, ignorable: true },
  ...
  async #receiveWeave(frame) {
    ...
    if (payload.kind === "session.catalog") return { claimed: true, result: await this.sessionCatalog() };
  ...
  async #acceptInvite(payload) {
    ...
    this.state.rooms.push({ id, name: ensureText(payload.room.name, "room name"), createdAt: Date.now(), members: [], messages: [], pendingDeliveries: [], hostTicket, capability: ensureText(payload.capability, "room capability"), cursor: 0 }); await this.#save(); await this.ensureRoomSessions(); this.#emit({ kind: "room-linked", roomId: id });
  }
