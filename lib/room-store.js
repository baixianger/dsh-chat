...
export class DshChatService {
  ...
  attachWeave() { if (!this.unsubscribeWeave && this.#weave()) this.unsubscribeWeave = this.#weave().subscribe((frame) => this.#receiveWeave(frame)); }
  ...
  async #invite(room, member) {
    ...
    await weave.send({ ticket: member.ticket, from: `chat:${room.id}:invite`, to: WEAVE_TARGET, text: JSON.stringify({ protocol: WEAVE_PROTOCOL, kind: "room.invite", room: { id: room.id, name: room.name }, hostTicket }) });
  }
  async #readHost(room, limit) {
    const reply = await this.#request(room.hostTicket, { kind: "room.read", roomId: room.id, cursor: room.cursor ?? 0, limit });
    room.cursor = reply.cursor; await this.#save(); return reply.events;
  }
  async #postToHost(room, request) { return this.#request(room.hostTicket, { kind: "room.post", roomId: room.id, ...request }); }
  ...
  async #receiveWeave(frame) {
    ...
    if (payload.kind === "room.read") { const room = this.#room(payload.roomId); const cursor = Math.max(0, Number(payload.cursor) || 0); return { claimed: true, result: { cursor: room.messages.length, events: copy(room.messages.slice(cursor, cursor + Math.min(500, payload.limit ?? 100))) } }; }
    if (payload.kind === "room.post") { const message = await this.send({ roomId: payload.roomId, author: payload.author, text: payload.text, mentions: payload.mentions }); return { claimed: true, result: message }; }
  ...
  async #acceptInvite(payload) {
    ...
    this.state.rooms.push({ id, name: ensureText(payload.room.name, "room name"), createdAt: Date.now(), members: [], messages: [], hostTicket, cursor: 0 }); await this.#save(); this.#emit({ kind: "room-linked", roomId: id });
  }
}
