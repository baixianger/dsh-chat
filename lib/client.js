  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      function RoomTimeline({ node }) {
        ...
        const field = { width: "100%", minHeight: 42, boxSizing: "border-box", padding: "9px 12px", border: "1px solid var(--dsw-alias-border-l2, #ddd)", borderRadius: 10, background: "var(--dsw-alias-bg-layer-1, #fff)", color: "inherit", fontSize: 14, outline: 0 };
        const selectField = { ...field, height: 46, padding: "0 38px 0 13px", fontWeight: 560, cursor: "pointer" };
          ...
          settingsOpen && h("div", { role: "presentation", style: { position: "fixed", inset: 0, zIndex: 100, background: "rgba(18,20,24,.22)", display: "flex", justifyContent: "flex-end" } },
            h("aside", { role: "dialog", "aria-modal": true, "aria-label": "Room settings", style: { width: "min(470px, 94vw)", height: "100%", boxSizing: "border-box", padding: 26, overflow: "auto", background: "var(--dsw-alias-bg-layer-1, #fff)", boxShadow: "-12px 0 36px rgba(0,0,0,.12)" } },
              ...
              h("section", { style: { marginBottom: 28 } },
                ...
                h("label", { style: { display: "grid", gap: 5, marginBottom: 10, fontSize: 12 } }, "Host",
                  h("select", { value: selectedSource?.hostId ?? "local", onChange: (event) => { const hostId = event.target.value; const source = sources.find((item) => item.hostId === hostId); setSourceHostId(hostId); setWorkspaceId(source?.workspaces[0]?.id ?? ""); }, style: selectField }, sources.map((source) => h("option", { key: source.hostId, value: source.hostId }, source.hostId === "local" ? source.hostName : `${source.hostName} · Remote`)))
                ),
                h("label", { style: { display: "grid", gap: 5, marginBottom: 10, fontSize: 12 } }, "Workspace",
                  h("select", { value: selectedWorkspace?.id ?? "", onChange: (event) => setWorkspaceId(event.target.value), disabled: !selectedSource?.workspaces.length, style: selectField }, (selectedSource?.workspaces ?? []).map((workspace) => h("option", { key: workspace.id, value: workspace.id }, workspace.title)))
                ),
                h("input", { value: sessionQuery, onChange: (event) => setSessionQuery(event.target.value), placeholder: "Search sessions in this workspace", style: field }),
