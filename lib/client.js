window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const inject = ["slots", "connection", "conversationEvents", "sessions"];
    ...
    function apply(ctx) {
      ...
      const readSessionRows = () => {
        ...
        return snapshot.ids.map((id) => snapshot.byId[id]).filter((session) => session && !String(session.id).startsWith("dsh-chat-room-"));
      };
      const definition = {
        kind: "dsh-chat-room",
        target: "chat",
      ...
      function RoomTimeline({ node }) {
        ...
        const [sessionRows, setSessionRows] = React.useState(readSessionRows);
        ...
        React.useEffect(() => {
          const refresh = () => setSessionRows(readSessionRows());
          refresh();
        ...
        const addRemoteSession = async (host, session) => {
          try { await call("addMember", { roomId: room.roomId, member: { kind: "weave", sessionId: String(session.id), ticket: host.ticket } }); await refreshRoom(); }
          catch (cause) { setError(String(cause.message ?? cause)); }
        };
        const openSettings = () => {
          setSettingsOpen(true); setRemoteLoading(true);
        ...
        const selectableSessions = sessionRows.filter((session) => !memberIds.has(String(session.id)) && (!query || `${session.displayTitle} ${session.id}`.toLowerCase().includes(query)));
        ...
        return h("section", { style: { width: "min(780px, calc(100% - 32px))", margin: "0 auto", padding: "24px 0 36px" } },
          h("header", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 18, marginBottom: 32 } },
            ...
            h("button", { type: "button", onClick: openSettings, style: quietButton, "aria-label": "Room settings" }, h(GearIcon), h("span", null, "Settings"))
          ),
          ...
          settingsOpen && h("div", { role: "presentation", style: { position: "fixed", inset: 0, zIndex: 100, background: "rgba(18,20,24,.22)", display: "flex", justifyContent: "flex-end" } },
            h("aside", { role: "dialog", "aria-modal": true, "aria-label": "Room settings", style: { width: "min(420px, 92vw)", height: "100%", boxSizing: "border-box", padding: 24, overflow: "auto", background: "var(--dsw-alias-bg-layer-1, #fff)", boxShadow: "-12px 0 36px rgba(0,0,0,.12)" } },
              ...
              h("section", { style: { marginBottom: 28 } },
                h("h4", { style: { margin: "0 0 4px", fontSize: 14 } }, "Add local session"),
                h("p", { style: { margin: "0 0 10px", color: "var(--dsw-alias-text-secondary, #777)", fontSize: 12 } }, "Choose from sessions on this DSH host."),
                h("input", { value: sessionQuery, onChange: (event) => setSessionQuery(event.target.value), placeholder: "Search sessions", style: field }),
                h("div", { role: "list", "aria-label": "Available sessions", style: { display: "grid", gap: 6, maxHeight: 230, marginTop: 9, overflow: "auto" } },
                  selectableSessions.slice(0, 50).map((session) => h("button", { key: session.id, type: "button", role: "listitem", onClick: () => addMember(String(session.id)), style: { display: "grid", gridTemplateColumns: "34px minmax(0,1fr) auto", alignItems: "center", gap: 10, width: "100%", padding: "8px 9px", border: 0, borderRadius: 10, background: "var(--dsw-alias-bg-layer-2, #f6f7f8)", color: "inherit", textAlign: "left", cursor: "pointer" } },
                    h(Avatar, { id: String(session.id), label: session.displayTitle, size: 32 }),
                    h("span", { style: { minWidth: 0 } }, h("span", { style: { display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 13, fontWeight: 600 } }, session.displayTitle), h("small", { style: { display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "var(--dsw-alias-text-secondary, #777)" } }, session.running ? "Running" : String(session.id).slice(0, 20))),
                    h("span", { style: { color: "var(--dsw-alias-text-secondary, #777)", fontSize: 12 } }, "Add")
                  )),
                  selectableSessions.length === 0 && h("small", { style: { padding: "12px 4px", color: "var(--dsw-alias-text-secondary, #777)" } }, query ? "No matching sessions." : "All local sessions are already members.")
                )
              ),
              h("section", { style: { marginBottom: 28 } },
                h("h4", { style: { margin: "0 0 4px", fontSize: 14 } }, "Paired hosts"),
                h("p", { style: { margin: "0 0 10px", color: "var(--dsw-alias-text-secondary, #777)", fontSize: 12 } }, "Reachable Weave hosts and their sessions."),
                remoteLoading && h("small", { style: { color: "var(--dsw-alias-text-secondary, #777)" } }, "Looking for paired hosts…"),
                !remoteLoading && remoteHosts.length === 0 && h("div", { style: { padding: 12, borderRadius: 10, background: "var(--dsw-alias-bg-layer-2, #f6f7f8)" } }, h("div", { style: { fontSize: 13, fontWeight: 600 } }, "No reachable hosts"), h("small", { style: { color: "var(--dsw-alias-text-secondary, #777)" } }, "Pair hosts in Settings → Weave.")),
                h("div", { style: { display: "grid", gap: 14 } }, remoteHosts.map((host) => h("div", { key: host.peerId },
                  h("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 } }, h("strong", { style: { fontSize: 13 } }, host.hostName), h("small", { style: { color: "var(--dsw-alias-color-primary, #5870d8)" } }, "Remote · online")),
                  h("div", { role: "list", "aria-label": `${host.hostName} sessions`, style: { display: "grid", gap: 6 } }, host.sessions.filter((session) => !members.some((member) => member.kind === "weave" && member.ticket === host.ticket && member.sessionId === String(session.id))).slice(0, 30).map((session) => h("button", { key: session.id, type: "button", role: "listitem", onClick: () => addRemoteSession(host, session), style: { display: "grid", gridTemplateColumns: "34px minmax(0,1fr) auto", alignItems: "center", gap: 10, width: "100%", padding: "8px 9px", border: 0, borderRadius: 10, background: "var(--dsw-alias-bg-layer-2, #f6f7f8)", color: "inherit", textAlign: "left", cursor: "pointer" } },
                    h(Avatar, { id: `${host.peerId}:${session.id}`, label: session.title, size: 32, remote: true }),
                    h("span", { style: { minWidth: 0 } }, h("span", { style: { display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 13, fontWeight: 600 } }, session.title), h("small", { style: { color: "var(--dsw-alias-text-secondary, #777)" } }, `${host.hostName} · Remote`)),
                    h("span", { style: { color: "var(--dsw-alias-text-secondary, #777)", fontSize: 12 } }, "Add")
                  )))
                )))
              ),
              h("section", null,
                h("h4", { style: { margin: "0 0 4px", fontSize: 14 } }, "Connect by ticket"),
                h("p", { style: { margin: "0 0 10px", color: "var(--dsw-alias-text-secondary, #777)", fontSize: 12, lineHeight: 1.45 } }, "Pairing, relay, and Iroh identity are owned by dsh-weave. Manage them in Settings → Weave, or connect one session directly here."),
