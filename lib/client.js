      ...
      function GroupChatView({ sessionId }) {
        ...
        React.useEffect(() => {
          ...
          const poll = async () => {
            while (!stopped) {
              try {
                const next = await call("messages", { roomId: activeId, limit: 200, waitMs: active?.hostTicket ? 25_000 : 0 });
                if (!stopped) setMessages(next);
                if (!active?.hostTicket) await new Promise((resolve) => setTimeout(resolve, 1_000));
              } catch (cause) {
        ...
        const addRemoteMember = async (event) => {
          event.preventDefault(); if (!activeId || !remoteSessionId.trim() || !remoteTicket.trim()) return;
          try { await call("addMember", { roomId: activeId, member: { kind: "weave", sessionId: remoteSessionId, ticket: remoteTicket } }); setRemoteSessionId(""); setRemoteTicket(""); await refreshRooms(); }
          catch (cause) { setError(String(cause.message ?? cause)); }
        };
        const send = async (event) => {
          ...
          try {
            ...
            const message = await call("send", { request: { roomId: activeId, author: sessionId, text: draft, mentions } }); setDraft(""); setMessages((current) => [...current, message]); await refreshRooms();
          }
          catch (cause) { setError(String(cause.message ?? cause)); }
        };
        ...
        return h("div", { style: style.root },
          ...
          h("section", { style: style.main },
            ...
            active && h("form", { onSubmit: addRemoteMember, style: { ...style.form, borderTop: 0, flexDirection: "column" } }, h("input", { value: remoteSessionId, onChange: (event) => setRemoteSessionId(event.target.value), placeholder: "Remote session id", style: style.input }), h("input", { value: remoteTicket, onChange: (event) => setRemoteTicket(event.target.value), placeholder: "Iroh ticket (trust this node)", style: style.input }), h("button", { type: "submit", style: style.button }, "Add remote node")),
