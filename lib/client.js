window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function stateLabel(state) {
      return ({ idle: "Idle", running: "Running", waking: "Waking…", offline: "Offline", "host-offline": "Host offline", archived: "Archived", missing: "Missing", connecting: "Connecting…" })[state] ?? "Unknown";
    }

    function stateColor(state) {
      if (state === "idle") return "#28a745";
      if (state === "running" || state === "waking" || state === "connecting") return "var(--dsw-alias-color-primary, #5870d8)";
      if (state === "offline" || state === "host-offline") return "#9aa0a6";
      return "var(--dsw-alias-text-tertiary, #999)";
    }
    ...
    function apply(ctx) {
      ...
      function RoomTimeline({ node }) {
        ...
        React.useEffect(() => {
          ...
          const poll = async () => {
            while (!stopped) {
              try {
                const next = await call("messages", { roomId: room.roomId, limit: 200, waitMs: room.remote && !firstRead ? 25_000 : 0 });
                firstRead = false;
                if (!stopped) { setMessages(next); setError(""); await refreshRoom(); }
        ...
        React.useEffect(() => {
          ...
          refresh();
          void call("remoteSessions").then(setRemoteHosts).catch(() => {});
        ...
        const addMember = async (session, workspace) => {
          const alias = session.displayTitle ?? session.title ?? session.id;
          try { await call("addMember", { roomId: room.roomId, member: { kind: "session", sessionId: String(session.id), alias, workspaceTitle: workspace?.title } }); await refreshRoom(); }
          catch (cause) { setError(String(cause.message ?? cause)); }
        };
        const addRemoteSession = async (host, workspace, session) => {
          const alias = session.displayTitle ?? session.title ?? session.id;
          try { await call("addMember", { roomId: room.roomId, member: { kind: "remote", hostId: host.hostId, hostName: host.hostName, sessionId: String(session.id), alias, workspaceTitle: workspace?.title } }); await refreshRoom(); }
          catch (cause) { setError(String(cause.message ?? cause)); }
        };
        const removeMember = async (member) => {
          try { await call("removeMember", { roomId: room.roomId, member: { kind: member.kind, sessionId: member.sessionId, ...(member.kind === "remote" ? { hostId: member.hostId } : {}) } }); await refreshRoom(); }
          catch (cause) { setError(String(cause.message ?? cause)); }
        };
        const openSettings = () => {
        ...
        const memberName = (member) => {
          const local = ctx.sessions.list.getSnapshot().byId[member.sessionId];
        ...
        const authorName = (message) => message.authorAlias ?? memberName(members.find((member) => member.sessionId === message.author) ?? { kind: "session", sessionId: message.author });
        ...
        return h("section", { "data-dsh-chat-room-timeline": "", style: { width: "min(780px, calc(100% - 32px))", margin: "0 auto", padding: "24px 0 36px" } },
            ...
            messages.map((message) => h("article", { key: message.id, style: { display: "grid", gridTemplateColumns: "38px minmax(0,1fr)", gap: 12, alignItems: "start" } },
              ...
              h("div", { style: { minWidth: 0, paddingTop: 1 } },
                h("div", { style: { display: "flex", alignItems: "baseline", gap: 8, marginBottom: 5 } },
                  h("strong", { title: authorName(message), style: { fontSize: 14, fontWeight: 650, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, authorName(message)),
            ...
            h("aside", { role: "dialog", "aria-modal": true, "aria-label": "Room settings", style: { width: "min(470px, 94vw)", height: "100%", boxSizing: "border-box", padding: 26, overflow: "auto", background: "var(--dsw-alias-bg-layer-1, #fff)", boxShadow: "-12px 0 36px rgba(0,0,0,.12)" } },
              ...
              h("section", { style: { marginBottom: 28 } },
                ...
                h("div", { style: { display: "grid", gap: 9 } }, members.map((member) =>
                  h("div", { key: `${member.kind}:${member.hostId ?? "local"}:${member.sessionId}`, title: member.sessionId, style: { display: "flex", alignItems: "center", gap: 10, padding: "8px 9px", borderRadius: 10, background: "var(--dsw-alias-bg-layer-2, #f6f7f8)" } },
                    ...
                    h("div", { style: { minWidth: 0, flex: 1 } },
                      ...
                      h("small", { style: { color: "var(--dsw-alias-text-secondary, #777)" } }, member.kind === "remote" ? `${member.workspaceTitle ?? "Workspace"} · ${member.hostName ?? "Remote via Weave"}` : member.workspaceTitle ?? "Local session")
                    ),
                    h("small", { style: { display: "inline-flex", alignItems: "center", gap: 5, color: "var(--dsw-alias-text-secondary, #777)", flex: "0 0 auto" } },
                      h("span", { style: { width: 7, height: 7, borderRadius: "50%", background: stateColor(runtimeState(member)) } }),
                      stateLabel(runtimeState(member))
                    ),
                    h("button", { type: "button", onClick: () => removeMember(member), "aria-label": `Remove ${memberName(member)} from room`, title: "Remove from room", style: { flex: "0 0 auto", width: 26, height: 26, borderRadius: "50%", border: 0, background: "transparent", color: "var(--dsw-alias-text-tertiary, #999)", font: "inherit", fontSize: 15, lineHeight: 1, cursor: "pointer", display: "inline-flex", alignItems: "center", justifyContent: "center" } }, "×")
                  )
                )),
              ...
              h("section", { style: { marginBottom: 28 } },
                ...
                h("div", { role: "list", "aria-label": "Available sessions", style: { display: "grid", gap: 6, maxHeight: 260, marginTop: 9, overflow: "auto" } },
                  selectableSessions.slice(0, 50).map((session) => { const title = session.displayTitle ?? session.title ?? session.id; const remote = selectedSource.hostId !== "local"; return h("button", { key: session.id, type: "button", role: "listitem", onClick: () => remote ? addRemoteSession(selectedSource, selectedWorkspace, session) : addMember(session, selectedWorkspace), style: { display: "grid", gridTemplateColumns: "34px minmax(0,1fr) auto", alignItems: "center", gap: 10, width: "100%", padding: "8px 9px", border: 0, borderRadius: 10, background: "var(--dsw-alias-bg-layer-2, #f6f7f8)", color: "inherit", textAlign: "left", cursor: "pointer" } },
                    ...
                    h("span", { style: { display: "inline-flex", alignItems: "center", gap: 6, color: "var(--dsw-alias-text-secondary, #777)", fontSize: 12 } }, h("span", { style: { width: 7, height: 7, borderRadius: "50%", background: stateColor(runtimeState(session)) } }), stateLabel(runtimeState(session)))
                  ); }),
      ...
      function RoomComposer({ matched, sessionId }) {
        ...
        React.useEffect(() => {
          let stopped = false;
          const refresh = async () => {
            try { const rooms = await call("listRooms"); if (!stopped) setMembers(rooms.find((room) => room.id === matched.roomId)?.members ?? []); }
        ...
        const mentionable = members.filter((member) => member.sessionId !== String(sessionId) && runtimeState(member) !== "archived");
        const insertMention = (member) => {
          const token = `@${nameOf(member)}`;
          setDraft((value) => `${value}${value && !/\s$/.test(value) ? " " : ""}${token} `);
          setSelectedMentions((value) => value.some((item) => item.sessionId === member.sessionId) ? value : [...value, { sessionId: member.sessionId, token }]);
        };
        const send = async (event) => {
          event.preventDefault(); if (!draft.trim() || sending) return;
          setSending(true);
          try {
            const selected = selectedMentions.filter((item) => draft.includes(item.token)).map((item) => item.sessionId);
            ...
            await call("send", { request: { roomId: matched.roomId, author: String(sessionId), authorAlias, text: draft, mentions } });
        ...
        return h("div", { style: { width: "min(780px, calc(100% - 32px))", margin: "0 auto", padding: "10px 0 18px" } },
          ...
          h("form", { onSubmit: send, className: "dshChatComposer", style: { padding: "12px 12px 9px", border: "1px solid var(--dsw-alias-border-l2, #ddd)", borderRadius: 19, background: "var(--dsw-alias-bg-layer-1, #fff)", boxShadow: "0 2px 8px rgba(0,0,0,.035), 0 12px 26px rgba(0,0,0,.035)" } },
            mentionable.length > 0 && h("div", { "aria-label": "Mention a session", style: { display: "flex", gap: 6, padding: "0 2px 9px", overflowX: "auto" } }, mentionable.map((member) => {
              const state = runtimeState(member);
              const label = nameOf(member);
              return h("button", { key: `${member.hostId ?? "local"}:${member.sessionId}`, type: "button", onClick: () => insertMention(member), title: `${label} · ${stateLabel(state)}`, "aria-label": `Mention ${label}, ${stateLabel(state)}`, style: { flex: "0 0 auto", display: "inline-flex", alignItems: "center", gap: 6, border: "1px solid var(--dsw-alias-border-l2)", borderRadius: 14, background: "var(--dsw-alias-bg-layer-2)", color: "var(--dsw-alias-label-secondary)", padding: "3px 9px", font: "inherit", fontSize: 12, cursor: "pointer" } },
            ...
            h("textarea", { value: draft, onChange: (event) => { const value = event.target.value; setDraft(value); setSelectedMentions((items) => items.filter((item) => value.includes(item.token))); }, rows: 2, placeholder: `Message ${matched.name}`, style: { width: "100%", minHeight: 44, maxHeight: 160, resize: "vertical", boxSizing: "border-box", padding: "2px 4px 8px", border: 0, outline: 0, background: "transparent", color: "inherit", font: "inherit", lineHeight: 1.45 } }),
