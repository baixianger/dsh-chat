...
export const Config = Schema.object({
  path: Schema.string().description("Durable room state file."),
  workspacePath: Schema.string().description("Workspace used for room sessions.")
});
...
export const inject = ["connection", "tools", "dshBridge", "dshWeave", "sessions", "workspaceRegistry", "sessionTitle"];
...
export function apply(ctx, config) {
  ...
  const handlers = {
    ...
    removeMember: ({ roomId, member }) => remote.removeMember(roomId, member),
    send: ({ request }) => remote.send(request)
  };
  ctx.inject(["webServer"], (webCtx) => {
    webCtx.connection.rpc.handle("/dsh-chat", async (endpoint, payload) => {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const room = await remote.chat.createRoom({ name: args.name, members: [{ kind: "session", sessionId, alias: owningAlias(exec) }] });
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      await remote.chat.addMember(room.id, { kind: "session", sessionId: owningSession(exec), alias: owningAlias(exec) });
      return { id: room.id, name: room.name };
    }
  }));
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      const room = await remote.chat.resolveRoom(args.room);
      const status = await ctx.dshBridge.status(args.sessionId);
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const message = await remote.chat.send({ roomId: room.id, author: sessionId, authorAlias: owningAlias(exec), text: args.text, mentions: args.mentions });
